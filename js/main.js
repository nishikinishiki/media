const App = {
    ...
    renderRankingTable() {
        ...
        const bodyHTML = rankingData.map((company, i) => `
            <tr class="${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} border-b hover:bg-amber-50">
                ...
                <td class="px-6 py-4 text-center align-middle"><a href="${company.detailsLink}" class="cta-button text-white font-bold py-2 px-4 rounded-full text-sm block whitespace-nowrap">公式サイトで詳細を見る</a></td>
            </tr>`
        ).join('');
    ...
    renderDetailsSection() {
        const detailsHTML = this.data.rankingData.map(company => `
            <article id="${company.id}" class="details-article bg-white p-6 rounded-lg shadow-lg scroll-mt-20">
                <div class="max-w-5xl mx-auto">
                    <div class="details-rank-title-box flex justify-center items-center gap-4 mb-6">
                    ...
                    <img src="${company.logoUrl}" alt="${company.companyName} ロゴ" class="details-logo mx-auto h-16 mb-6">
                    ...
                    ${company.displayOptions?.showReviews ? `<h4 class="details-section-title text-2xl font-bold text-center mb-6 border-b-2 border-amber-500 pb-2">口コミ</h4><div class="review-card-container flex overflow-x-auto space-x-6 pb-4 mb-10">${(company.reviews||[]).map(r=>`<div class="review-card flex-none w-80 bg-gray-50 p-6 rounded-lg shadow"><div class="flex items-center mb-4"><i class="fas fa-user-circle text-4xl text-gray-400 mr-3"></i><div><p class="font-bold">${r.author}</p></div></div><p class="text-gray-700 italic">「${r.quote}」</p>${r.source ? `<p class="text-right text-xs text-gray-500 mt-4">- ${r.source}より引用</p>`:''}</div>`).join('')}</div>` : ''}
